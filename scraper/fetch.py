...
Field confirmed: "Instrument Type", "Date (From)", "Date (To)"
"""
...
def compute_flags(r, now):
    flags, cat, dt, owner = [], r.get("cat",""), r.get("doc_type",""), (r.get("owner") or "").upper()
    ...
    try:
        if r.get("filed") and (now-datetime.strptime(r["filed"],"%Y-%m-%d")).days<=7: flags.append("New this week")
    except: pass
    return flags
...
def blank_rec(doc_code, cat, cat_label):
    return {"doc_num":"","doc_type":doc_code,"filed":"","cat":cat,"cat_label":cat_label,
...
class ClerkScraper:
    ...
    async def _fill_dates(self, page):
        ...
        for label_text, value in [("Date (From)", df_str), ("Date (To)", dt_str)]:
            try:
                await page.get_by_label(label_text).fill(value, timeout=4000)
                continue
            except: pass
            # Fallback: find input near that label text
            try:
                await page.locator(f"text={label_text}").locator("xpath=following::input[1]").fill(value, timeout=3000)
            except: pass

    async def _search_rp(self, page, code, cat, label):
        log.info(f"Searching RP: {code}")
        await page.goto(RP_URL, timeout=60000, wait_until="domcontentloaded")
        await page.wait_for_timeout(2000)

        # Fill Instrument Type field
        filled = False
        for approach in [
            lambda: page.get_by_label(re.compile("instrument type", re.I)).fill(code, timeout=4000),
            lambda: page.get_by_label(re.compile("type", re.I)).fill(code, timeout=4000),
            lambda: page.locator("text=Instrument Type").locator("xpath=following::input[1]").fill(code, timeout=3000),
            lambda: page.locator("input[name*='nstrument']").fill(code, timeout=3000),
            lambda: page.locator("input[name*='Type']").fill(code, timeout=3000),
            lambda: page.locator("#ctl00_cphMain_txtInstrumentType").fill(code, timeout=3000),
        ]:
            try:
                await approach()
                filled = True
                log.info(f"  [{code}] Instrument Type filled")
                break
            except: pass

        if not filled:
            # Last resort: dump all input names to help debug
            inputs = await page.locator("input[type='text']").all()
            names = []
            for inp in inputs:
                n = await inp.get_attribute("name") or await inp.get_attribute("id") or "?"
                names.append(n)
            log.warning(f"[{code}] Could not fill type. Inputs found: {names}")
            return

        await self._fill_dates(page)

        # Submit
        for sel in ["input[type='submit']","button[type='submit']","input[value*='Search']","input[value*='search']"]:
    ...
    async def _search_foreclosures(self, page):
        """Scrape the dedicated foreclosure notices page."""
        ...
        for label_text, value in [("Date (From)", df_str), ("Date (To)", dt_str),
            ...
            try:
                await page.get_by_label(label_text).fill(value, timeout=3000)
            except: pass
    ...
    async def _paginate(self, page, code, cat, label):
        ...
        while True:
            ...
            try:
                nxt = page.locator("a:has-text('Next'), input[value='Next >'], a[title='Next Page']").first
    ...
    def _parse(self, html, code, cat, label):
        ...
        for tbl in soup.find_all("table"):
            ...
            joined=" ".join(hdrs)
            if not any(k in joined for k in ("doc","filed","grantor","instrument","grantee","date","file")): continue
            col={}
            for i,h in enumerate(hdrs):
                if any(x in h for x in ("doc num","instrument","file num","film")): col.setdefault("doc_num",i)
                elif "filed" in h or h in ("date","recorded"): col.setdefault("filed",i)
                elif "grantor" in h or "debtor" in h or "seller" in h: col.setdefault("grantor",i)
                elif "grantee" in h or "creditor" in h or "buyer" in h: col.setdefault("grantee",i)
                elif "legal" in h or "description" in h or "subdiv" in h: col.setdefault("legal",i)
                elif "amount" in h or "consid" in h: col.setdefault("amount",i)
                elif "type" in h: col.setdefault("type_col",i)
            if not col: continue
            for row in rows[1:]:
                ...
                try:
                    ...
                    doc_num=t("doc_num") or t("type_col")
                    if not doc_num or doc_num.lower() in ("no records","none"): continue
                    # doc link
                    url=""
                    for cell in cells:
        ...
        if not dbf.exists():
            for url in self.URLS:
                try:
                    ...
                    log.info(f"Parcel downloaded")
                    break
                except Exception as e: log.warning(f"Parcel DL: {e}")
        if not dbf.exists(): return False
        try:
            # Use ignore_missing_memofile and handle unknown field types gracefully
            count=0
            for row in DBF(str(dbf),encoding="latin-1",ignore_missing_memofile=True,raw=False):
                try:
                    row=dict(row)
                    owner=self._col(row,["OWNER","OWN1","OWNER_NAME","OWNERNM"])
                    if not owner: continue
                    for v in self._variants(owner): self._idx.setdefault(v,row)
                    count+=1
                except: continue
            log.info(f"Parcel index: {count:,} records, {len(self._idx):,} keys")
            return True
        except Exception as e:
            log.error(f"DBF error: {e}")
            return False
...
def write_output(records,df,dt):
    payload={"fetched_at":datetime.utcnow().isoformat()+"Z",
        "source":"Harris County Clerk","date_range":{"from":df.strftime("%Y-%m-%d"),"to":dt.strftime("%Y-%m-%d")},
...
async def main():
    ...
    log.info(f"Range: {df.date()} → {dt.date()}")
    ...
    seen=set(); unique=[]
    for r in records:
        if r["doc_num"] not in seen: seen.add(r["doc_num"]); unique.append(r)
    records=unique
